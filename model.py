...
class RoofEnsemble(nn.Module):
	def __init__(self):
		...
		self.mlp = nn.Sequential(nn.Linear(2, 64),
									...
									# nn.Dropout(0.25),
									# nn.Linear(256, 256))
		#self.classifier = nn.Sequential(nn.Linear(2048*1 + 64*0, 5))
		self.classifier = nn.Sequential(nn.Linear(25088*1 + 64*1, 512),
									nn.LeakyReLU(0.3),
									nn.Dropout(0.5),
									nn.Linear(512, 64),
									nn.LeakyReLU(0.3),
	...
	def init_branches(self):
		...
		model_roof = models.vgg16_bn(pretrained=True)
		model_context = models.vgg16_bn(pretrained=True)
